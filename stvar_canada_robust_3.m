...
% (4) Estimation of Smooth-transitioning VAR, using a different uncertainty
% measure, namely the one-year-ahead macroeconomic uncertainty
...
%Specification 4 (Change to uncertainty one year ahead)
vars = {'Unc_US_h3','y','u','CPI','Unc_CAN_h3'}; % only for labelling, data is loaded directly
printvars = {'US Uncertainty (one year)','Real GDP growth','Unemployment rate','Inflation (CPI)','CAN Uncertainty (one year)'}; 

data = table2array(data_lib(:,["mean_US_unc_h12","Real_GDP_CAN","UNEMP_CAN","CPI_CAN","CAN_unc_h4"])); 
...
shockpos = 5;  %CAN uncertainty shock = 5
...
save(strcat(folder, '/out_4_can.mat'),  'STVAR', 'VAR','printvars','folder') %CAN Uncertainty shock
