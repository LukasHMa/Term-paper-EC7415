...
% (4) Estimation of Smooth-transitioning VAR, according to Eq.(5)
...
vars = {'Unc_US','y','u','CPI','Unc_CAN'}; % only for labelling, data is loaded directly
printvars = {'US Uncertainty','Real GDP growth','Unemployment rate','Inflation (CPI)','CAN Uncertainty'}; 

data = table2array(data_lib(:,["mean_US_unc_h1","Real_GDP_CAN","UNEMP_CAN","CPI_CAN","CAN_unc_h1"])); 
...
%shockpos = 5; %CAN uncertainty shock = 5 (Baseline ordering, Alternative ordering 4ab)
...
writetable(tab,'./Results/InfC_baseline_1.xlsx') 
...
save(strcat(folder, '/out_1_us.mat'), 'STVAR', 'VAR','printvars','folder') %US Uncertainty shock
%save(strcat(folder, '/out_1_can.mat'), 'STVAR', 'VAR','printvars','folder') %CAN Uncertainty shock