% Author: Lukas Ma (modified from codes by Gabriel Züllig)
%
% Date: 2022-12-29
%
% A function that plots the IRFs. Modified from the plotirf3 function in
% the macroeconometric toolbox written by Gabriel Züllig
% 


function plotirf3_mod(irf1, irfbands1, irf2, irfbands2, irf3, irfbands3, printvars, path, printstate, poslegend)
...
figure()
tcl = tiledlayout(nrow,3,'TileSpacing','Compact');
for vv = 1:n
    ...
    if vv == n && ~isempty(printstate)
        LL = legend([p1 p2 p3], printstate,'FontSize',12);
        LL.Layout.Tile = poslegend;
    end
    ax = gca;
    ax.YAxis.Exponent = 0;
...
print(gcf,'-depsc2','-loose',strcat(path,'_irf'));