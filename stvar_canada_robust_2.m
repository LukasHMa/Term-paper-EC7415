...
% (4) Estimation of Smooth-transitioning VAR, using an alternative ordering that corresponds to Eq.(6)
...
%Specification 3b (Canadian uncertainty is ordered second, use job finding probability)
vars = {'Unc_US','Unc_CAN','y','pr','CPI'}; % only for labelling, data is loaded directly
printvars = {'US Uncertainty','CAN Uncertainty','Real GDP growth','Job finding probability','Inflation (CPI)'}; 

data = table2array(data_lib(:,["mean_US_unc_h1","CAN_unc_h1","Real_GDP_CAN","mean_PROB_JOB_FINDING","CPI_CAN"])); 
...
writetable(tab,'./Results/InfC_robust_2.xlsx') 
...
%save(strcat(folder, '/out_3b_us.mat'), 'STVAR', 'VAR','printvars','folder') %US Uncertainty shock
save(strcat(folder, '/out_3b_can.mat'), 'STVAR', 'VAR','printvars','folder') %CAN Uncertainty shock
